    ...
    for i in range(1,len(rows)):
        ...
        if(rows[i][2] == date):
            ...
            if(date[5:] != "02/29"):
                ...
                number += 1
                print(date[5:]+","+str(number/count),file = sample)
...
def plot_moving_average(series, series2, window, metric, plot_intervals=False, scale=1.96):
    ...
    plt.plot(rolling_mean2, label='Rolling mean trend 2019')
    plt.plot(rolling_mean, label='Rolling mean trend 2020')
    ...
    # plt.plot(series2[window:], label='Actual values for 2019')
    # plt.plot(series[window:], label='Actual values for 2020')
    plt.legend(loc='best')
    plt.grid(True)
    # plt.show()
    plt.savefig(subreddit + "-MovingAverage.png" )
...
def plot_exponential_smoothing(series, alphas, metric):
    ...
    plt.grid(True);
    # plt.show()
    plt.savefig(subreddit + "-ExponentialSmoothing.png" )
...
def plot_double_exponential_smoothing(series, metric, alphas, betas):
    ...
    plt.grid(True)
    # plt.show()
    plt.savefig(subreddit + "-DoubleExponentialSmoothing.png" )
...
def tsplot(y, metric, lags=None, figsize=(12, 7), syle='bmh'):
    ...
    # plt.show()
    plt.savefig(subreddit + "-TSPlot.png" )
...
# axes.plot(data3[metric])  
axes.legend(["2019", "2020"], loc ="upper right") 
axes.set(xlabel="Date", ylabel="Number of posts" ,title="Number of posts in 2019 vs 2020 ")

plt.savefig(subreddit + "-Comparison.png" )
# plt.show()
